...
let validated_rgb (c :color) : color =
   match c with
   |Simple a -> Simple a
   | RGB (x,y,z) -> if x>=0 && x<=255 && y>=0 && y<=255 && z>=0 && z<=255 then RGB (x,y,z) else
   raise (Invalid_color "not a valid color");;
...
let convert_to_rgb (c: color) : int*int*int =
  ...
  |RGB (r,g,b)-> if validate_rgb (RGB(r,g,b)) == RGB(r,g,b) then (r,g,b);;
